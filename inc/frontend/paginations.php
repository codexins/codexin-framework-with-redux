...
if ( ! function_exists( 'codexin_posts_nav' ) ) {
    ...
    * @since    v1.0.0
    */
    function codexin_posts_nav( $prev = 'Newer Posts', $next = 'Older Posts', $custom = NULL ) {
        $prev_link = get_previous_posts_link( '&laquo; '. $prev );
        $next_link = ( $custom !== NULL ) ? get_next_posts_link( esc_html__( $next, 'TEXT_DOMAIN' ). ' &raquo; ', $custom->max_num_pages ) : get_next_posts_link( esc_html( $next, 'TEXT_DOMAIN' ). ' &raquo; ' );
